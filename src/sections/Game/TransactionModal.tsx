...
export function TransactionModal(props: { onClose: () => void }) {
  ...
  const initialize = async () => {
    try {
      ...
      await sendTransaction(program.methods.playerInitialize().instruction(), {
  ...
  const closeAccount = async () => {
    try {
      ...
      await sendTransaction(program.methods.playerClose().instruction(), {
        confirmation: "confirmed",
      });
    } finally {
    ...
    <Modal onClose={() => props.onClose()}>
      ...
      <div style={{ display: "flex", gap: "10px" }}>
        ...
        {txStore.txId && (
          <GambaUi.Button
            main
            onClick={() => window.open("https://solscan.io/tx/" + txStore.txId)}
          >
            View TX
          </GambaUi.Button>
        )}
